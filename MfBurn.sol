...
// Root file: contracts/MfBurn.sol
...
contract MfBurn is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier noContractAllowed(address addr) {
        require(
            !address(addr).isContract() && addr == tx.origin,
    ...
    function burn(uint256 amount, address user)
        external
        nonReentrant
        noContractAllowed(msg.sender)
    {
